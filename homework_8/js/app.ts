...
class Game {
    ...
    score;
    moves: number = 0;
    wins: Array<String> = ["1 2 4","8 16 32","64 128 256","1 16 256","4 16 64","1 8 64","1 16 128","4 32 256"];
    gameNumber: number = 0;
    player1 = {name: "player1", symbol: "X", score: 0};
    player2 = {name: "player2", symbol: "O", score: 0};
    activeCell;
    activePlayer;
    ...
    win(score: string) {
        let source = score.split(",");
        for (var i = 0; i < this.wins.length; i++) {
            let arr = this.wins[i].split(" ");
            let count = 0;
	...
	set(cell) {
        ...
        let smbl = this.activePlayer.symbol;
    ...
    up() {
        let id = this.activeCell.id;
        let newRowNumber = Number(id[0]) - 1;
        let newColNumber = id[1];
